   ...
   if($request->answers){
      foreach($request->answers as $answer){		
         ...
         if ($request->user_id==Yii::app()->user->id && $hole->STATE == Holes::STATE_GIBDDRE)
            echo ' '.CHtml::link(Yii::t('template', 'EDIT'), Array('gibddreply','id'=>$hole->ID,'answer'=>$answer->id), Array('class'=>'declarationBtn'));
         echo CHtml::closeTag('h2');
         
			if ($answer->files_other){
            foreach($answer->files_other as $file){
               echo CHtml::openTag('p',array('class'=>'holes_pict_p'));  
